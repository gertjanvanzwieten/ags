...
def dump(f, obj, T):
    yaml.safe_dump(_mapping.mapping_for(T).lower(obj, _Backend), f, **_dump_settings)


def dumps(obj, T):
    return yaml.safe_dump(
        _mapping.mapping_for(T).lower(obj, _Backend), **_dump_settings
    )


def load(f, T):
    return _mapping.mapping_for(T).unlower(yaml.safe_load(f), _Backend)


def loads(s, T):
    return _mapping.mapping_for(T).unlower(yaml.safe_load(s), _Backend)

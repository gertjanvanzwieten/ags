    ...
    def unlower(obj, T):
        ...
        if T is str:
            ...
            for si in _split_exposed(obj, ","):
                ...
                if pos == -1:
                    raise ValueError(
                        f"dictionary item {si!r} does not contain an '=' sign"
                    )
...
def dump(f, obj, T):
    f.write(_mapping.mapping_for(T).lower(obj, _Backend))
    f.flush()


def dumps(obj, T):
    return _mapping.mapping_for(T).lower(obj, _Backend)


def load(f, T):
    return _mapping.mapping_for(T).unlower(f.read(), _Backend)


def loads(s, T):
    return _mapping.mapping_for(T).unlower(s, _Backend)
